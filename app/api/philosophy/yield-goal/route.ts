...
const EDGE_CONFIG_KEY = 'yield-goal';
...
// GET - Fetch yield goal
export async function GET() {
  try {
    const value = await getEdgeConfigValue(EDGE_CONFIG_KEY);
    return NextResponse.json({ value: value || '' });
  } catch (error) {
    console.error('Error fetching yield goal from Edge Config:', error);
    return NextResponse.json({ value: '' });
  }
}

// POST - Save yield goal
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error saving yield goal:', error);
    return NextResponse.json(
      { error: 'Failed to save yield goal' },
      { status: 500 }
    );
  }
}
